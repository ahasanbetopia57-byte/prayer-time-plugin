...
class CityLocationPrayer
{
    ...
    public function display_city_location_prayer($atts = [])
    {
        ...
        $countries_json_path = $plugin_dir . 'data/countries.json';

        // Read and decode JSON file
        $countries_data = '{"countries": []}'; // Default empty data
        if (file_exists($countries_json_path)) {
            $json_content = file_get_contents($countries_json_path);
            $decoded_data = json_decode($json_content, true);

            if (json_last_error() === JSON_ERROR_NONE && is_array($decoded_data)) {
                // Properly encode for JavaScript
                $countries_data = wp_json_encode($decoded_data);
            }
        }
?>
        <style>
            .month-table tr.today-highlight {
                background: #3b82f6;
                color: white;
            }
        ...
        <script>
            ...
            // Load countries data from external JSON file
          let countries = <?php echo $countries_data; ?>;
            ...
            async function selectLocation(city, country) {
                ...
                document.getElementById('locationTextInHero').textContent = 'Prayer times for ' + city + ', ' + country;
                closeLocationModal();
                ...
                if (!selectedLocation) {
                    ...
                    try {
                        ...
                        if (data.code === 200) {
                            ...
                            // Format the hijri date (e.g., "4th Sha'ban, 1447")
                            const hijriDay = hijri.day;
                            const hijriMonth = hijri.month.en;
                            const hijriYear = hijri.year;

                            document.getElementById('dateInfo').innerHTML =
                                '<div>' + hijriDay + ' ' + hijriMonth + ', ' + hijriYear + '</div>' +
                        ...
                        document.getElementById('dateInfo').innerHTML =
                            '<div>4th Sha\'ban, 1447</div>' + // Fallback
                            '<div>' + day + ' ' + month + ', ' + year + '</div>';
                    }
                    return;
                }
            ...
            function renderMonthView() {
                ...
                if (!selectedLocation) {
                    // Show current month placeholder with today highlighted
                    const now = new Date();
                    ...
                    let rowsHTML = '';
                    let hijriCounter = 12; // Starting hijri day from your image
                    ...
                    for (let day = 1; day <= daysInMonth; day++) {
                        ...
                        // Format hijri day (simple incrementing for placeholder)
                        const hijriDay = hijriCounter;
                        hijriCounter = hijriCounter >= 30 ? 1 : hijriCounter + 1;

                        rowsHTML += '<tr class="' + (isToday ? 'today-highlight' : 'month-placeholder') + '">' +
                            '<td>' + (day < 10 ? '0' + day : day) + ' ' + weekday + '</td>' +
                            '<td>' + hijriDay + '</td>' +
                            '<td>---</td>' +
                            '<td>---</td>' +
                            '<td>---</td>' +
                    ...
                    content.innerHTML = '<h3 style="margin-bottom: 20px; color: #4b5563;">' + monthName + ' ' + currentYear + '</h3>' +
                ...
                const today = new Date();
                const todayStr = today.toISOString().split('T')[0].split('-'); // Get as array [YYYY, MM, DD]
                const todayDay = parseInt(todayStr[2]); // Get day number

                let rowsHTML = '';
                monthData.forEach(function(day) {
                    const gregorianDate = day.date.gregorian.date;
                    const gregorianParts = gregorianDate.split('-'); // Split into [DD, MM, YYYY]
                    const dayNum = parseInt(gregorianParts[0]); // Get day number

                    // Check if this is today by comparing day number
                    const isToday = (dayNum === todayDay);

                    const weekday = day.date.gregorian.weekday.en.substring(0, 3);

                    rowsHTML += '<tr class="' + (isToday ? 'today-highlight' : '') + '">' +
                        '<td>' + day.date.gregorian.day.padStart(2, '0') + ' ' + weekday + '</td>' +
                ...
                // Add month header when location is selected
                const monthHeader = monthData[0] ?
                    monthData[0].date.gregorian.month.en + ' ' + monthData[0].date.gregorian.year :
                    '';

                content.innerHTML = '<h3 style="margin-bottom: 20px; color: #4b5563;">' + monthHeader + '</h3>' +
                    '<table class="month-table">' +
                    '<thead>' +
                    '<tr>' +
                    '<th>Date</th>' +
            ...
            // Initialize
            updateDateInfo(); // Show current date on load
            renderTodayPlaceholder();
